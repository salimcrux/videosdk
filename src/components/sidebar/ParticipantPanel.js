    ...
    return (
      <>
        ...
        >
          ...
          >
            {!isLocal && canRemoveOtherParticipant && <ParticipantRemoveIcon />}
          </button>
        </div>
        <div
          style={{ zIndex: 999 }}
          className={`${
            tooltipShow ? "" : "hidden"
          } overflow-hidden flex flex-col items-center justify-center pb-1 mr-4`}
          ref={tooltipRef}
        >
          <div className={"rounded-md p-1.5 bg-black "}>
            <p className="text-base text-white ">{"Remove"}</p>
          </div>
        </div>
      </>
    );
