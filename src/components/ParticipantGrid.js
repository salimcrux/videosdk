...
function ParticipantGrid({ participantIds, isPresenting, sideBarMode }) {
  const { promoText, promoUrl } = useMeetingAppContext(); // Salim
  console.log('Text11: ',promoText);
  console.log('Url11: ',promoUrl);
  ...
  const { participantMode, isLandscape } = useMeetingAppContext();

  const perRow =
    (isMobile && !isLandscape) || isPresenting
      ? participantIds.length < 4
        ? 1
        : participantIds.length < 9
        ? 2
        : 3
      : !gtThanMD && !isLandscape
      ? participantIds.length < 4
        ? 1
        : participantIds.length < 9
        ? 2
                ...
                {participantIds
                  ...
                  .map((participantId) => {
                    return (
                      ...
                      >
                        {participantId == "NULL" ? (
                          <PromoInfographic title={promoText} url={promoUrl}/>
                          // <PromoImage />
                        ) : (
                          <MemoizedParticipant
