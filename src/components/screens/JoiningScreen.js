...
export function JoiningScreen({
  ...
  const [{ webcams, mics, outputmics }, setDevices] = useState({
    devices: [],
    webcams: [],
    mics: [],
    outputmics: [],
  ...
  const getDefaultMediaTracks = async ({ mic, webcam, outputmics, firstTime }) => {    
    if (mic) {
  ...
  const getDevices = async ({ micEnabled, webcamEnabled }) => {
    try {
      ...
      const hasOutputMics = outputmics.length > 0;
      
      setDevices({ webcams, mics, outputmics, devices });
      ...
      getDefaultMediaTracks({
        ...
        outputmics: hasOutputMics && micEnabled,
        firstTime: true,
      });
    } catch (err) {
      console.log(err);
  ...
  useEffect(() => {
    ...
    //   theme: "light",
    // });
    window.parent.postMessage({type:"notify", info:"Join screen loaded"}, '*'); // Salim
  }, []);
  ...
  return (
    <>
      <div className="overflow-y-auto flex flex-col flex-1 h-screen bg-white">
        <div className="flex flex-1 flex-col md:flex-row items-center justify-center md:m-[72px] ml-16 mr-16 mt-9 mb-0">
          <div className="container grid  md:grid-flow-col grid-flow-row ">
            <div className="grid grid-cols-12" >
              ...
              {/* <div className="flex items-center justify-center  p-1.5 sm:p-4 lg:p-4 md:p-3 xl:p-4 mt-0 2xl:mt-4 xl:mt-1 lg:mt-1 md:mt-0"> */}
              
              <div className="flex items-center justify-center  p-1.5 sm:p-4 lg:p-4 md:p-3 xl:p-4 mt-0 2xl:mt-4 xl:mt-1 lg:mt-1 md:mt-0">
                <div className="w-full relative" style={{ height:"45vh"}} >
                <div className="flex flex-1 flex-col  items-center justify-center   sm:m-9 m-3 sm:mt-0 mt-0   xl:m-0 lg:m-0 md:m-0 md:mt-0 lg:mt-0 xl:mt-0   md:absolute md:left-0 md:right-0 md:top-0 md:bottom-0">
                  <MeetingDetailsScreen  
                    changeMic={changeMic} 
                    mics={mics}   
                    outputmics={outputmics}  // Salim
                    audioTrack={audioTrack} 
                    webcams={webcams}  
                    setSelectedWebcam={setSelectedWebcam} 
                    changeWebcam={changeWebcam} 
                    ...
                    onClickJoin={async (id) => {
                      ...
                      if (valid) {
                        ...
                        onClickStartMeeting();
                        window.parent.postMessage({type:"notify", info:"Join Meeting Button Clicked !"}, '*'); // added by Salim
